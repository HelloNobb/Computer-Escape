...
function scene:create( event )
	...
	local image_rose = display.newImage("image/오브제/rose0_glass.png")
	...
	local content = display.newText(dialog, "여긴 어디지..?", display.contentWidth*0.5, display.contentHeight*0.902, display.contentWidth*0.7, display.contentHeight*0.2, font_Content)
	...
	-- json에서 정보 읽기
	local Data = jsonParse("rose_json/rose_pi.json")
	...
	local function nextScript( event )
		...
		if(index > #Data) then --뷰 끝나면 다음씬으로!
			local function goToNextScene()
                ...
                composer.gotoScene("rose_game", options)
            end
			goToNextScene()
			return
		end
