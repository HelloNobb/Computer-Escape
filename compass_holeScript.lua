...
function scene:create( event )
	...
	local image_pi2 = display.newImage("image/캐릭터/파이 당황.png")
	image_pi2.x, image_pi2.y = display.contentWidth*0.2, display.contentHeight*0.5
	image_pi2.name = "파이"
	image_pi2.alpha = 0

	local image_pi3 = display.newImage("image/캐릭터/파이 결심.png")
	image_pi3.x, image_pi3.y = display.contentWidth*0.2, display.contentHeight*0.5
	image_pi3.name = "파이"
	image_pi3.alpha = 0
	...
	local content = display.newText(dialog, "나침반 대로 오긴 했는데...", display.contentWidth*0.5, display.contentHeight*0.902, display.contentWidth*0.7, display.contentHeight*0.2, font_Content)
	...
	sceneGroup:insert(image_pi3)
	sceneGroup:insert(talk1)
	sceneGroup:insert(speaker)
	sceneGroup:insert(content)

	-- json에서 정보 읽기
	local Data = jsonParse("json/compass_holeScript.json")
...
function scene:hide( event )
	...
	if event.phase == "will" then
		...
		composer.removeScene( "compass_holeScript" )
	elseif phase == "did" then
