...
function scene:create( event )
	...
	local image_pi = display.newImage("image/캐릭터/파이 당황.png")
	...
	local content = display.newText(dialog, "또 뭘 해야하지...?", display.contentWidth*0.5, display.contentHeight*0.902, display.contentWidth*0.7, display.contentHeight*0.2, font_Content)
	content:setFillColor(0)
	content.size = 40
	--content.font = "font/PF스타더스트.ttf"

	local quest = display.newText(dialog, " ", display.contentWidth*0.5, display.contentHeight*0.84, display.contentWidth*0.7, display.contentHeight*0.2, font_Speaker)
	content:setFillColor(0)
	content.size = 45
	...
	-- json에서 정보 읽기
	local Data = jsonParse("rose_json/rose_pi2.json")
	...
	local function nextScript( event )
		...
		if(index > #Data) then --뷰 끝나면 다음씬으로!
			local function goToNextScene()
                ...
                composer.gotoScene("rose_ending", options)
            end
			goToNextScene()
			return
		end
...
function scene:hide( event )
	...
	if event.phase == "will" then
		...
		composer.removeScene("rose_outro_dialog")
	elseif phase == "did" then
